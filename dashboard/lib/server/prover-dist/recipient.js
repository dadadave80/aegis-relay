...
export async function deriveRecipientKey(seedHex) {
...
// ── CLI ─────────────────────────────────────────────────────────────────────
async function cmdSignPod(flags) {
    ...
    if (!packetPath || !id || !carrierCommit || flags.lat === undefined || flags.lon === undefined) {
        throw new Error('usage: sign-pod --packet <path> --id <n> --carrier-commit <decimal> --lat <deg> --lon <deg> [--ts <unix>]');
    }
