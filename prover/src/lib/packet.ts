...
export interface Packet {
  ...
  recipient_claim: { eddsa_seed_hex: string };
  lane_id?: number;
  corridor_ref?: string;
}
...
const spkiDer = (key: crypto.KeyObject): Buffer =>
  key.export({ type: 'spki', format: 'der' }) as Buffer;

function deriveKeyNonce(shared: Buffer, salt: Buffer): { key: Buffer; nonce: Buffer } {
...
export function sealX25519(recipientPubPem: string, obj: unknown): Sealed {
  ...
  return {
    v: PACKET_VERSION,
    alg: SEAL_ALG,
    epk: eph.publicKey.export({ type: 'spki', format: 'pem' }) as string,
    ct: ct.toString('hex'),
...
export function openX25519(privPem: string, sealed: Sealed): unknown {
  if (sealed.alg !== SEAL_ALG) throw new Error(`unsupported seal alg: ${sealed.alg}`);
...
export function shipDir(id: string | number): string {
...
export function writePacket(id: string | number, packet: Packet): string {
...
export function readPacket(path: string): Packet {
...
export function writeSealedPacket(
  id: string | number,
  recipientPubPem: string,
  packet: Packet,
): string {
