...
function degScaled(deg: string | number): bigint {
...
const clampQ = (q: bigint): bigint => (q < 0n ? 0n : q > Q_MAX ? Q_MAX : q);

/** lat_q = floor((lat_deg + 90) / 180 · 2^24), exact integer math (§5.3). */
export function latToQ(latDeg: string | number): bigint {
...
export function lonToQ(lonDeg: string | number): bigint {
  const s = degScaled(lonDeg);
  return clampQ(((s + 180n * Q_SCALE) * (1n << 24n)) / (360n * Q_SCALE));
}

export function latLonToQ(
  latDeg: string | number,
  lonDeg: string | number,
): { latQ: bigint; lonQ: bigint } {
  return { latQ: latToQ(latDeg), lonQ: lonToQ(lonDeg) };
}

/** Top `r` bits of a 24-bit quantized coordinate (bits 23 .. 24-r). */
export function topBits(q: bigint, r: number): bigint {
 ...
 */
export function mortonFromTop(latTop: bigint, lonTop: bigint, r: number): bigint {
  let cell = 0n;
...
export function mortonCell(latQ: bigint, lonQ: bigint, r: number): bigint {
  return mortonFromTop(topBits(latQ, r), topBits(lonQ, r), r);
}

// ── Destination region (3×3 RD-cell grid → depth-6 padded tree) ─────────────
 ...
 */
export function destRegionGridCells(
  latQ: bigint,
  lonQ: bigint,
): { cells: bigint[]; centerIndex: number } {
...
export async function buildDestRegionTree(
  latQ: bigint,
  lonQ: bigint,
): Promise<DestRegionTree> {
  const { cells, centerIndex } = destRegionGridCells(latQ, lonQ);
  const p = await getPoseidon();
  const leaf = (cell: bigint): string =>
    p.F.toString(p([DOM_CELL, cell]));
...
// ── Fixed-depth padded Merkle builder ───────────────────────────────────────
 ...
 */
export async function buildFixedTree(
  realLeaves: Array<bigint | string>,
  depth: number,
): Promise<FixedTree> {
  const slots = 1 << depth;
  ...
  const p = await getPoseidon();
  const h2 = (a: string, b: string): string =>
    p.F.toString(p([BigInt(a), BigInt(b)]));
  const PAD = h2('0', '0');

  const leaves: string[] = realLeaves.map((l) => BigInt(l).toString());
  while (leaves.length < slots) leaves.push(PAD);
  ...
  const paths: FixedTreePath[] = realLeaves.map((_, index) => {
