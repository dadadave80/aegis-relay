...
import { computeCS, custodyHead, nullifier, pkCommit } from './lib/poseidon.js';
import { mortonCell } from './lib/tree.js';
import { buildInvoke, parseFlags, proofToInvokeJson, resolveRegistryId, runInvoke, SOURCE, TESTNET, } from './lib/contract.js';
import { OUT_ROOT, readPacket, writePacket, shipDir } from './lib/packet.js';
...
export function openingToShipment(o) {
    return {
        skuHash: o.sku_hash,
        qty: o.qty,
        weightG: o.weight_g,
 ...
 */
export function sampleFieldSalt() {
    let n = 0n;
...
export async function carrierPkCommit(key) {
    return pkCommit(key.pkX, key.pkY, key.pkBlind);
}
export async function verifyPacket(packet, onchainCs) {
...
// ── CLI ─────────────────────────────────────────────────────────────────────
function printOrRun(argv, registryId) {
    if (registryId) {
...
async function cmdVerifyPacket(flags) {
    if (!flags.packet)
        throw new Error('usage: verify-packet --packet <path> [--onchain-cs <decimal>]');
...
async function cmdAccept(flags) {
    ...
    if (packet.shipment_id)
        writePacket(packet.shipment_id, packet);
    const id = flags.id ?? packet.shipment_id;
    if (!id)
        throw new Error('shipment id unknown: pass --id or create the packet with one');
...
async function cmdDeliver(flags) {
    if (!flags.id)
        throw new Error('usage: deliver --id <n> [--registry <C...>]');
    const registryId = resolveRegistryId(flags.registry);
    const dir = shipDir(flags.id);
    const proof = JSON.parse(readFileSync(join(dir, 'proof.json'), 'utf8'));
    const publicSignals = JSON.parse(readFileSync(join(dir, 'public.json'), 'utf8'));
