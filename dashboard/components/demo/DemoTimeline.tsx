...
/**
 * Lifecycle timeline for the console, driven by the pinned demo
 * `ShipmentView` (lib/types). Visual language mirrors the site-wide
 * components/ShipmentTimeline exactly — same glyphs, colours and connector —
 * but reads the demo shape (string state, "courier"/"drone" method) instead of
 * the raw on-chain decode used by /track.
 */
...
function buildSteps(s: ShipmentView): Step[] {
  const accepted =
    s.head !== null || s.state === "IN_TRANSIT" || s.state === "DELIVERED";
...
export default function DemoTimeline({ shipment }: { shipment: ShipmentView }) {
