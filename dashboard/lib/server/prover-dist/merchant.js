...
import { buildInvoke, parseFlags, resolveRegistryId, runInvoke, SOURCE, TESTNET, } from './lib/contract.js';
import { generatePacketKeypair, PACKET_VERSION, shipDir, writePacket, writeSealedPacket, } from './lib/packet.js';
import { openingToShipment, sampleFieldSalt } from './carrier.js';
import { deriveRecipientKey } from './recipient.js';
const DAY = 86_400;
export async function buildShipment(p) {
 ...
 */
export function buildRefundInvoke(args) {
    return buildInvoke({
        fn: 'refund_expired',
        args: [['id', args.id]],
        source: args.source ?? SOURCE.merchant,
...
async function cmdCreate(flags) {
    if (flags['to-lat'] === undefined || flags['to-lon'] === undefined || !flags.amount || !flags['deadline-hours']) {
        throw new Error('usage: create --to-lat <deg> --to-lon <deg> --amount <i128> --deadline-hours <n> [--method courier|drone] [--lane <u32>] [--id <n>]');
    }
    const method = flags.method ?? 'courier';
...
async function cmdRefund(flags) {
    if (!flags.id)
        throw new Error('usage: refund --id <n> [--registry <C...>]');
    const registryId = resolveRegistryId(flags.registry);
