...
components/console/config.ts}` and `docs/SUBMISSION.md`.

### CT re-pin (2026-07-03) — confidential rail now pinned to the current registry

To make the confidential rail live against the role-binding registry, a NEW CT
stack was deployed pinned to `CAROLAUW…` (the old token `CAIRUFAA…` is
permanently pinned to the previous registry — pins are set-once). Deployed by
`prover/scripts/deploy-ct-repin.mjs` (source `relay-admin`, Alchemy RPC); 6
UltraHonk VKs registered, auditor key id 0 set, addr_f parity OK, and
`registry.set_ct_token` closed the pin (verified: `ct_token()` returns the new
token).

| Contract | ID |
|---|---|
| aegis-ct-token (hooked, pinned to `CAROLAUW…`) | `CCKUW6LFVRZZ7AYQ3HXW3MP4CYW4ZZO6LZRIPKUH7MX2GJVXPWYHYOHC` |
| CT verifier (6 UltraHonk VKs) | `CCZIHLG6KBNVRRM2QKP6HFSLJXV356ASA2XHPCRIDXNB6PAEZ7QGO756` |
| CT auditor (regulator key id 0) | `CDCOI5E6FZQGUPD2DSYA6EEYFFXPYUHHYV3FSSG3N3LWWD3WZVYU3SNN` |

Wired into `dashboard/lib/server/artifacts.ts` + `components/console/config.ts`.
This is Phase 0 of advisor plan 004 (wire the confidential rail into the app);
the client-side confidential lifecycle (Phases A–D) builds on it.
