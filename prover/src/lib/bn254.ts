...
export function toBE32(value: bigint | string): Uint8Array {
  ...
  if (n < 0n) throw new Error(`Value ${value} is negative`);
  const out = new Uint8Array(32);
  for (let i = 31; i >= 0; i--) {
    out[i] = Number(n & 0xffn);
    n >>= 8n;
  }
  if (n !== 0n) throw new Error(`Value ${value} does not fit in 32 bytes`);
  return out;
}
 ...
 */
export function encodePublics(publicSignals: Array<string | bigint | number>): string[] {
...
export function bigintToU256ScVal(value: bigint | string): xdr.ScVal {
  const n = typeof value === 'bigint' ? value : BigInt(value);
  if (n < 0n || n >= 1n << 256n) throw new Error(`Value ${value} out of U256 range`);
...
export function proofToScVal(proof: EncodedProof): xdr.ScVal {
  const entry = (key: string, bytes: Uint8Array) =>
    new xdr.ScMapEntry({
