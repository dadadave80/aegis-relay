...
import { encodeProof } from './bn254.js';
/**
 ...
 */
export function buildInvoke(cmd) {
    ...
    for (const [name, value] of cmd.args) {
        if (value === undefined)
            continue; // Option None → omit flag
 ...
 */
export function runInvoke(argv) {
    const [bin, ...rest] = argv;
    ...
    if (!ok) {
        throw new Error(`stellar invoke failed (exit ${res.status}):\n` +
...
export function proofToInvokeJson(proofJson) {
 ...
 */
export function resolveRegistryId(explicit) {
