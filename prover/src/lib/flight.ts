...
export function topsFromMorton(cell: bigint, r: number): { latTop: bigint; lonTop: bigint } {
  let latTop = 0n;
  let lonTop = 0n;
 ...
 * identical).
 */
export function corridorCellCover(
  oLatQ: bigint,
  oLonQ: bigint,
  dLatQ: bigint,
  dLonQ: bigint,
  ...
  if (cells.length > 2 ** CORRIDOR_DEPTH) throw new Error('corridor overflows the depth-12 tree');
  return cells;
}

/** Build the depth-12 PAD-filled Poseidon tree over Poseidon(DOM_CELL, cell) leaves. */
export async function buildCorridorTree(cells: bigint[]): Promise<FixedTree> {
  const leaves: string[] = [];
  for (const c of cells) leaves.push(await cellLeaf(c));
 ...
 */
export async function authorCorridor(
  laneId: number,
...
export async function deriveDroneKey(seedHex: string, pkBlind: string | bigint): Promise<DroneKey> {
...
export async function signField(seedHex: string, msg: string | bigint): Promise<EddsaSig> {
  const eddsa = await getEddsa();
  const bjF = eddsa.babyJub.F;
...
// ── Waypoint interpolation + pre-prover asserts (§5.5 trap) ──────────────────
...
export function interpolateWaypoints(
  ...
  dLatQ: bigint,
  dLonQ: bigint,
  opts: { n?: number; t0: bigint; dt: bigint; altDm: bigint },
): WaypointArrays {
 ...
 */
export function preAssertLog(wp: WaypointArrays, weightG: bigint): void {
  const n = wp.t.length;
  for (let i = 1; i < n; i++) {
    const ti = BigInt(wp.t[i]);
    const tp = BigInt(wp.t[i - 1]);
    if (ti <= tp) throw new Error(`non-monotonic t at ${i}`);
    const dt = ti - tp;
    if (dt > BigInt(GAP_MAX_SEC)) throw new Error(`gap ${dt} > GAP_MAX at ${i}`);
    ...
    if (lhs > rhs) throw new Error(`SPEED BOUND VIOLATED at pair ${i}: ${lhs} > ${rhs}`);
  }
  for (const a of wp.alt_dm) if (BigInt(a) > BigInt(ALT_MAX_DM)) throw new Error('altitude bust');
  if (weightG > BigInt(DRONE_MAX_G)) throw new Error('overweight');
}

// ── The running flight digest d_16 (DESIGN §8.3) ─────────────────────────────

/** d_16 over shipment_id + the 16 waypoints (Poseidon chain), decimal string. */
export async function computeFlightDigest(
  shipmentId: string | bigint,
  wp: WaypointArrays,
): Promise<string> {
  const digests = await flightDigest(
    shipmentId,
    wp.lat_q.map((_, i) => ({
      latQ: wp.lat_q[i],
      lonQ: wp.lon_q[i],
 ...
 */
export async function buildFlightScenario(p: FlightScenarioParams): Promise<FlightScenario> {
  ...
  const corridorPath: string[][] = [];
  const corridorPathIndex: string[][] = [];
  for (let i = 0; i < n; i++) {
    const c = mortonCell(BigInt(wp.lat_q[i]), BigInt(wp.lon_q[i]), RC_RES).toString();
    const idx = corridorIndex.get(c);
    if (idx === undefined) throw new Error(`waypoint ${i} cell ${c} not in corridor`);
  ...
  // Commitments.
  const fullOpening: ShipmentOpening = { ...p.opening, originCell, destRegionRoot };
...
// ── Attack modes (mutation semantics == circuits/test/flight.test.mjs) ───────
...
export const ATTACK_NOTES: Record<AttackMode, string> = {
...
async function resign(
  w: FlightWitness,
  seedHex: string,
): Promise<FlightWitness> {
  const wp: WaypointArrays = {
    lat_q: w.lat_q as string[],
    lon_q: w.lon_q as string[],
    alt_dm: w.alt_dm as string[],
    t: w.t as string[],
  };
  const d16 = await computeFlightDigest(w.shipment_id as string, wp);
...
export async function applyAttack(
  ...
  switch (mode) {
    ...
    case 'stray': {
      ...
      const lonQ = BigInt((honest.lon_q as string[])[i]) + 3n * 2n ** BigInt(24 - RC_RES);
      const lon_q = (honest.lon_q as string[]).map((v, k) => (k === i ? lonQ.toString() : v));
    ...
    case 'teleport': {
      ...
      const latQ = BigInt((honest.lat_q as string[])[i]) + 900n;
      const newCell = mortonCell(latQ, BigInt((honest.lon_q as string[])[i]), RC_RES);
      const cellIdx = ctx.scenario.corridorCells.findIndex((c) => c === newCell);
      if (cellIdx === -1) throw new Error('teleport target cell is not in the corridor');
      const path = ctx.scenario.corridorTree.paths[cellIdx];
      const lat_q = (honest.lat_q as string[]).map((v, k) => (k === i ? latQ.toString() : v));
      const corridor_path = (honest.corridor_path as string[][]).map((p, k) =>
        k === i ? path.pathElements : p,
      );
      const corridor_index = (honest.corridor_index as string[][]).map((p, k) =>
        k === i ? path.pathIndices.map(String) : p,
      );
    ...
    case 'gap': {
      ...
      const t = (honest.t as string[]).map((v, k) =>
        k < i ? v : (BigInt((honest.t as string[])[i - 1]) + 45n + 20n * BigInt(k - i)).toString(),
      );
    ...
    case 'nonmono': {
      ...
      const t = (honest.t as string[]).map((v, k) =>
        k < i ? v : (BigInt((honest.t as string[])[i - 1]) - 5n + 20n * BigInt(k - i)).toString(),
      );
      return resign({ ...honest, t, t_n: t[t.length - 1] }, drone);
    }
    ...
    case 'heavy': {
      ...
      const cs = await computeCS({
        skuHash: honest.sku_hash as string,
        qty: honest.qty as string,
        weightG: heavy,
        valueUnits: honest.value_units as string,
        originCell: honest.origin_cell as string,
        destRegionRoot: honest.dest_region_root as string,
        recipientPkX: honest.recipient_pk_x as string,
        recipientPkY: honest.recipient_pk_y as string,
        method: honest.method as string,
        deadlineTs: honest.deadline_ts as string,
        shipmentSecret: honest.shipment_secret as string,
      });
    ...
    case 'altitude': {
      ...
      const alt_dm = (honest.alt_dm as string[]).map((v, k) => (k === i ? '1500' : v));
    ...
    default: {
      ...
      throw new Error(`unknown attack mode: ${_exhaustive as string}`);
    }
  }
}
